...
// 4종 게임(돌림판, 셔플, 슬롯, 줄뽑기) 실행 화면
// solo/random, food/random 양쪽에서 사용
export default function FlowRandomPage({
  backHref,
  sessionKeys,
  resultHref,
}: FlowRandomPageProps) {
  ...
  useEffect(() => {
    ...
    const loc = session.get<string>(sessionKeys.location);
    setCandidates(c);
    setLocation(loc);
  ...
  }, [sessionKeys.candidates, sessionKeys.location]);

  async function handleResult(winner: Candidate) {
    if (done) return;
    setDone(true);
    session.set(sessionKeys.activity, winner);
    try {
      ...
      session.set(sessionKeys.resultId, result.id);
    } catch {
      // DB 저장 실패해도 결과는 보여줌
    }
  ...
  // 테스트 모드: 게임 타입 선택 화면 (candidates 로드 완료 후에만 표시)
