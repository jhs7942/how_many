# Google Play Console MCP 자동 빌드 및 배포 설정

## 학습 환경
- 날짜: 2026-04-16 / 관련 프로젝트: how_many (몇명이니) / 기술·버전: Google Play Console MCP, Capacitor, Gradle, uvx

---

## 배경
Android 앱(Capacitor 기반)을 Google Play Console에 배포할 때 매번 수동으로 AAB 빌드 → Play Console UI 업로드를 반복하는 것이 비효율적이었다. Claude Code 세션에서 한 번의 명령으로 빌드부터 업로드까지 자동화하고, 테스트(Internal) → 프로덕션 단계적 롤아웃까지 제어하고 싶었다.
...
Claude Code가 외부 도구/API를 호출할 수 있게 해주는 프로토콜. `.mcp.json`에 서버를 등록하면 Claude가 해당 도구를 직접 사용할 수 있다.
...
`AgiMaulana/GooglePlayConsoleMcp`를 선택. `upload_artifact` 도구가 APK/AAB 자동 감지 + 트랙 릴리즈 생성을 원자적으로 처리한다.

### Google Play 트랙 구조

| 트랙 | 심사 | 전파 시간 | 용도 |
|---|---|---|---|
| **Internal testing** | 사실상 없음 | 수분 | 스모크 테스트 (최대 100명) |
| Closed testing (α) | 있음 | 수시간~1일 | 비공개 그룹 테스트 |
| Open testing (β) | 있음 | 1~3일 | 공개 베타 |
| **Production** | 있음 | 1~7일 | 정식 배포 |

### 배포 전략: Internal → Production staged rollout

```
AAB 빌드 → Internal 업로드 (심사 없음, 수분 내 설치)
→ 본인 기기 테스트
→ Production 10% rollout → 24~48시간 Vitals 모니터링
→ 50% → 100% 순차 확대
```

1인 개발에서 Closed/Open testing은 오버스펙. Internal에서 충분히 검증 후 Production staged rollout이 실질적 안전망.

### Android Studio 없이 CLI 빌드 가능 여부

**가능하다.** 필요한 것은 JDK + Android SDK + Gradle wrapper뿐. Android Studio GUI는 불필요.
...
`JAVA_HOME`이 Android Studio 내장 JBR을 가리키지만, Studio 앱을 실행하는 것이 아니라 JDK만 참조.

---

## 실제 적용

### 1. MCP 서버 등록 (`.mcp.json`)
...
### 3. Play Console 권한 부여

"API 액세스" 메뉴가 최신 Play Console에서 사라짐. 대안:
- **사용자 및 권한** → **신규 사용자 초대** → 서비스 계정 이메일 입력
- 권한: 앱 정보 보기, 출시 관리, 프로덕션 출시

### 4. SA 키 유출 방지 (`.gitignore`)
...
채택: **build.gradle 직접 수정** 방식. 배포 시 Claude가 Edit 도구로 versionCode +1 수정.
...
배포 시 `git log` → Claude가 한국어 변환 → 사용자 확인 후 MCP 전달.
...
- **SA JSON 키 발급 후 24~36시간** 후에 정상 동작할 수 있음 (Google 전파 지연)
...
- **첫 업로드는 수동 필수** — API는 이미 등록된 앱에만 업로드 가능. 패키지명 생성 + 첫 릴리즈는 Play Console UI에서
